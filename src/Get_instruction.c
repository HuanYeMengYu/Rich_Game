...
void Get_instruction(char* keyboard_input){
    Player nowplayer;
    if(now_user[0]==1) nowplayer = QianFuren;
    else if(now_user[0]==2) nowplayer = ATubo;
    else if(now_user[0]==3) nowplayer = SunXiaomei;
    else if(now_user[0]==4) nowplayer = JinBeibei;
    ...
    if (strcmp(command, "Roll") == 0) {
        get_gold(&nowplayer, get_step_count());//完成掷骰子和位置更新
